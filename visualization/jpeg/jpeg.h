...
static void create_huffman_tab(jpeg_decompr *jd, const uint8_t *data, size_t size) {
	...
	while(size) {
		...
		jd->huffman_distr[tab_num][tab_class] = pb;
		for(np = i = 0; i < 16; ++i) {
			pb[i]= *data++;
			np += pb[i];
		}
		...
		jd->huffman_data[tab_num][tab_class] = pd;
		for(i = 0; i < np; ++i) {
			d = *data++;
			if(!tab_class && d > 11) {
...
static int extract_bits(jpeg_decompr *jd, unsigned int length) {
	...
	while(wbit < length) {
		if(jd->marker) {
			...
			if(!data_counter) {
				data_pointer = jd->bit_stream_input;
				data_counter = jd->input_func(jd, data_pointer, JPEG_INPUT_BUFFER_SIZE);
				if(!data_counter) {
					printf("JPEG: Device error or wrong stream termination\n");
					exit(1);
				}
...
static void restart(jpeg_decompr *jd, uint16_t restart_interval) {
	unsigned int i;
	uint8_t *data_ptr = jd->data_read_ptr;
	size_t data_counter = jd->input_data_available;
	...
	for(;;) {
		...
		switch(marker & 0xff) {
			...
			case 0xDD:
				...
				jd->restart_interval = LBD_WORD(seg);
				break;
